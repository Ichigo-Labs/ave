...
- Fixed Windows pnpm global install detection to recognize `\\.pnpm\\` store paths, so update notices now suggest `pnpm install -g @ichigo.moe/ave` instead of falling back to npm ([#3378](https://github.com/badlogic/pi-mono/issues/3378))
- Fixed missing `@sinclair/typebox` runtime dependency in `@ichigo.moe/ave`, so strict pnpm installs no longer fail with `ERR_MODULE_NOT_FOUND` when starting `pi` ([#3434](https://github.com/badlogic/pi-mono/issues/3434))
...
- Fixed missing root exports for `RpcClient` and RPC protocol types from `@ichigo.moe/ave`, so ESM consumers can import them from the main package entrypoint ([#3275](https://github.com/badlogic/pi-mono/issues/3275))
...
- Fixed monorepo `tsconfig.json` path mappings to resolve `@ichigo.moe/ave/ai` subpath exports to source files in development checkouts ([#2625](https://github.com/badlogic/pi-mono/pull/2625) by [@ferologics](https://github.com/ferologics))
...
- Added missing `ajv` direct dependency; previously relied on transitive install via `@ichigo.moe/ave/ai` which broke standalone installs ([#2252](https://github.com/badlogic/pi-mono/issues/2252))
...
- Built-in tools now work like custom tools in extensions. To get built-in tool definitions, import `readToolDefinition` / `createReadToolDefinition()` and the equivalent `bash`, `edit`, `write`, `grep`, `find`, and `ls` exports from `@ichigo.moe/ave`.
...
- Added `ToolCallEventResult` to the `@ichigo.moe/ave` top-level and core extension exports so extension authors can type explicit `tool_call` handler return values ([#2458](https://github.com/badlogic/pi-mono/issues/2458))
...
- Faster startup by lazy-loading `@ichigo.moe/ave/ai` provider SDKs on first use instead of import time ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Lazy-load built-in `@ichigo.moe/ave/ai` provider modules and root provider wrappers so coding-agent startup no longer eagerly loads provider SDKs before first use ([#2297](https://github.com/badlogic/pi-mono/issues/2297))
...
- Fixed `@ichigo.moe/ave/ai/oauth` extension imports in published installs by resolving the subpath directly from built `dist` files instead of package-root wrapper shims ([#1856](https://github.com/badlogic/pi-mono/issues/1856)).
...
- Moved Node OAuth runtime exports off the top-level `@ichigo.moe/ave/ai` entry. OAuth login and refresh must be imported from `@ichigo.moe/ave/ai/oauth` ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed extension loading with the new `@ichigo.moe/ave/ai/oauth` export path by aliasing the oauth subpath in the extension loader and development path mapping ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
...
- Fixed browser bundling compatibility for `@ichigo.moe/ave/ai` by removing Node-only side effects from default browser import paths ([#1814](https://github.com/badlogic/pi-mono/issues/1814)).
## [0.55.4] - 2026-03-02
...
- Extensions now load correctly in compiled Bun binary using `@mariozechner/jiti` fork with `virtualModules` support. Bundled packages (`@sinclair/typebox`, `@ichigo.moe/ave/tui`, `@ichigo.moe/ave/ai`, `@ichigo.moe/ave`) are accessible to extensions without filesystem node_modules.
...
```typescript
// Before (hook)
import type { HookAPI, HookContext } from "@ichigo.moe/ave";
export default function (pi: HookAPI) { ... }

// Before (custom tool)
import type { CustomToolFactory } from "@ichigo.moe/ave";
const factory: CustomToolFactory = (pi) => ({ name: "my_tool", ... });
export default factory;

// After (both are now extensions)
import type { ExtensionAPI } from "@ichigo.moe/ave";
...
- **Key detection functions removed from `@ichigo.moe/ave/tui`**: All `isXxx()` key detection functions (`isEnter()`, `isEscape()`, `isCtrlC()`, etc.) have been removed. Use `matchesKey(data, keyId)` instead (e.g., `matchesKey(data, "enter")`, `matchesKey(data, "ctrl+c")`). This affects hooks and custom tools that use `ctx.ui.custom()` with keyboard input handling. ([#405](https://github.com/badlogic/pi-mono/pull/405))
...
- `Attachment` type removed. Use `ImageContent` from `@ichigo.moe/ave/ai` instead. Add images directly to message content arrays.
...
- **SDK API changes** ([#296](https://github.com/badlogic/pi-mono/issues/296)):
  ...
  - Use `getModel()` from `@ichigo.moe/ave/ai` for built-in models, `modelRegistry.find()` for custom models + built-in models
...
- **OAuth and model config exports**: Scripts using `AgentSession` directly can now import `getAvailableModels`, `getApiKeyForModel`, `findModel`, `login`, `logout`, and `getOAuthProviders` from `@ichigo.moe/ave` to reuse OAuth token storage and model resolution. ([#245](https://github.com/badlogic/pi-mono/issues/245))
...
- **`getMarkdownTheme()` export**: Custom tools can now import `getMarkdownTheme()` from `@ichigo.moe/ave` to use the same markdown styling as the main UI.
...
- Hooks loader now supports same import aliases as custom tools (`@sinclair/typebox`, `@ichigo.moe/ave/ai`, `@ichigo.moe/ave/tui`, `@ichigo.moe/ave`).
...
- Fixed custom tools failing to load from `~/.pi/agent/tools/` when pi is installed globally. Module imports (`@sinclair/typebox`, `@ichigo.moe/ave/tui`, `@ichigo.moe/ave/ai`) are now resolved via aliases.
...
- Updated `@ichigo.moe/ave/ai` with X-Initiator header support for GitHub Copilot, ensuring agent calls are not deducted from quota. ([#200](https://github.com/badlogic/pi-mono/pull/200) by [@kim0](https://github.com/kim0))
...
- Updated `@ichigo.moe/ave/ai` with interleaved thinking enabled by default for Anthropic Claude 4 models.
...
- Updated `@ichigo.moe/ave/ai` with interleaved thinking support for Anthropic models.
